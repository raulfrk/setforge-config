...
- **[CRITICAL] Co-author substantive content; show before committing.** Specs, configs, rules, doc rewrites, code snippets > a few lines → write to a temp file, open revdiff for annotation, even outside plan mode. **Background sessions are no exception** — revdiff opens in the attached tmux session's popup, so never downgrade to an inline-only proposal just because the session is backgrounded. Mechanical edits (typos, formatting, obvious one-liners) skip this and land directly.
