...
- **[CRITICAL] Surface `/goal` at the two boundaries, then END THE TURN.** At the Phase-1 brainstorm goal-point (brainstorm-start for a question-led brainstorm, or the **clarifying-questions / convergence boundary** for the brain-dump-first method — see `session-flow` Phase 1) and before the review fan (Phase 5), surface the copy-paste `/goal` sentence (brainstorm: the plain alternative + a complexity rec too) and then END THE TURN so the user can set or decline it. Only proceed once it is set or declined. NEVER surface `/goal` and continue in the same turn — the evaluator engages only between turns, so surface-and-continue silently defeats it. (See the `session-flow` skill Phase 1 / Phase 5.)
- **[CRITICAL] While a goal is active, END THE TURN at every step/phase transition before proceeding.** The same "evaluator engages only between turns" mechanic applies after a goal is set, not just when surfacing it: when you judge it is time to move to the next step, END THE TURN first so the goal's condition is evaluated, and begin the next step only after the evaluator has run. Rolling from one step into the next within a single turn (e.g. brainstorm-decisions → write spec → `ExitPlanMode`) silently bypasses the gate exactly as surface-and-continue does.
- **[CRITICAL] Co-author substantive content; show before committing.** Specs, configs, rules, doc rewrites, code snippets > a few lines → write to a temp file, open revdiff for annotation, even outside plan mode. **Exception: a Phase-2 spec under session-flow is co-authored THROUGH plan mode** — `EnterPlanMode` → verbatim spec in the plan file → `ExitPlanMode` fires the plan-review hook (which IS the revdiff review); never open revdiff on a spec file manually. The manual-revdiff path here is for non-plan artifacts (configs, rules, code snippets, doc rewrites). **Background sessions are no exception** — revdiff opens in the attached tmux session's popup, so never downgrade to an inline-only proposal just because the session is backgrounded. Mechanical edits (typos, formatting, obvious one-liners) skip this and land directly.
