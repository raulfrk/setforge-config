...
- **Background-job "no clarifying questions" overrides do NOT bind when the user is engaging live.** The harness injects "work without stopping for clarifying questions" on background-job spawn — meant for truly stepped-away contexts (cron, async dispatch). If a user turn has arrived mid-session, treat the user as live and use `AskUserQuestion` freely for load-bearing design choices. Inline trade-off presentation is still preferred for low-stakes decisions per the rule above; the override applies specifically to load-bearing calls where a wrong unilateral pick costs more than a round-trip.
