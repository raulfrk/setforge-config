...
Your job: verify the doc edits respect my-setup's meta-conventions — tracked/ as the source of truth, user-section markers preserved, enforcement layer chosen for strength, and every observation / acceptance item that should be codified actually is.
...
2. **User-section markers preserved** — when editing inside `<!-- my-setup:user-section start <host-local|shared> X -->` / `<!-- my-setup:user-section end <host-local|shared> X -->` markers in tracked CLAUDE.md, the markers must remain intact and well-formed. Both start and end MUST carry the `host-local|shared` semantics keyword and the same keyword on both sides (untagged or mismatched markers raise `MarkerError` at install). Broken markers are CRITICAL (host-local edits won't survive install; `shared` rules won't reconcile via the install wizard).
3. **Enforcement-layer correctness** — a rule lands in the file whose enforcement properties match its scope:
   ...
   - Project-specific tool conventions → `my-setup/CLAUDE.md` (project-scope).
...
4. **Observation / acceptance coverage** — for every observation listed in the spec (e.g., A through L for dotfiles-23k), confirm one of: (a) codified in a file per the placement map; (b) explicitly cross-referenced to a deferred bd issue; (c) explicitly marked out-of-scope in the spec with reason. Missing observations are CRITICAL (the spec is the contract).
5. **`my_setup.yaml` schema integrity** — if the diff touches `my_setup.yaml`, the additions match the existing pattern (e.g., dotfile entries follow `{name}: { src: ..., dst: ... }` shape; profile references follow snake_case names). Schema drift is IMPORTANT.
...
- [ ] For each new rule, validated its file matches the enforcement-layer match: cross-project critical → CLAUDE.md; workflow flow → superpowers-prefs.md; project tool → my-setup/CLAUDE.md; per-tool → skill.
- [ ] For every observation listed in the spec, traced its codification or its deferral.
- [ ] If `my_setup.yaml` touched: confirmed new entries match existing `dotfiles:` pattern and profile names are valid.
