...
This skill governs how every session runs — from start to close. Content survives compaction (re-attached up to 5,000 tokens).
...
Invoke the appropriate reviewing skill per artifact type:
- `reviewing-python-code` — Python source / pyproject / CI / pre-commit (5 aspect agents in parallel).
- `reviewing-claude-md` — tracked/claude/ docs / skills / agents (5 aspect agents).
- `reviewing-markdown` — generic .md files outside tracked/claude/ (1 prose agent).
Mixed artifacts invoke every applicable skill.
...
Re-invoke the same reviewing-X skill against merged HEAD on the target branch. Catches integration-emergent issues. Mandatory for multi-worktree; recommended for single-stream non-trivial.
...
2. If current bd is paused (in_progress but session ending) → propose creating a handoff. Or user invokes `/handoff`.
3. **Create handoff bead in `~/handoff/`** — ALWAYS in the handoff repo, NEVER in the current project's beads database (auto-inits `~/handoff/` as git repo + beads on first use):
   ...
   - `--description`: project dir path + active worktree path + branch + **epic ID** (for monorepo filtering)
   - `--notes`: free-form next-steps description (what to do first when resuming)
...
| 5 | `reviewing-*` (per artifact) | After implementation |
...
<!-- Per-host phase overrides, team review rules, branching conventions. -->
<!-- setforge:user-section end host-local host-local-workflow -->
