...
2. **Resolve the review surface** (do this once, up front). Read the `review-surface` setting — `atelier` (default) or `revdiff`. If unset, default to `atelier`; if `atelier` resolves but no browser/hub is reachable, fall back to `revdiff` (prompt only when genuinely ambiguous). Every later human review gate — Phase-1 mockup, Phase-2 plan-review, Phase-5/6 diff review, and learning mode — inherits this surface. See "Review surface" below.
...
1. Generate a **atelier** page populating each section's authored **`why`** with the teaching explanation — what changed, why, and the reasoning. This is the native fit: the rationale sits beside each hunk, per section. (No-browser fallback: write the diff to a temp file with inline explanatory annotations.)
...
When OFF, use the standard per-bead review offer (atelier default; Phase 6).
...
- **`atelier`** (default) — a served web page over the tailnet: each change with its authored rationale beside the syntax-highlighted hunk, embedded diagrams/charts, a section overview map, per-section reviewed-ticks with close-gating, inline annotations, and a Submit button that auto-resumes you. Best when rationale-beside-code, colour, diagrams, or phone/iPad review carry the meaning.
- **`revdiff`** — terminal TUI; the **no-browser fallback**, and the right pick for a fast in-terminal pass.

The `review-surface` setting (`atelier` | `revdiff`) is a **plain documented setting** read at session start — NOT a user-section and NOT external config. Resolution: use the set value; if unset, default to `atelier`; if `atelier` is selected but no browser/hub is reachable, fall back to `revdiff`. (The former `webdiff` value is still accepted as an alias for `atelier` for one cycle.) The user can override per session or per review. Both surfaces emit annotations in the same `## file:line (type)` markdown (one shared parser), so a single review can even hand off between them.

**Atelier also serves authored mockups** (terminal emulations, sub-sites, full websites, design comps), not just diffs — the same hub, annotation runtime, and hide-annotations toggle. See the `atelier` skill for mockup authoring + the CSS kit.
