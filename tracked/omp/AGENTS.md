...
Use `openai-codex/gpt-5.6-sol` at `medium` reasoning effort as the default
primary. Use the configured `worker` role at `max` for bounded implementation
and the built-in `scout` through `@smol` for read-only exploration. Preserve
configured reviewer roles, reasoning efforts, and review requirements. Explicit
user selections take precedence. Choose the worker mix by task difficulty;
start with one or two independent lanes and expand only when the decomposition
supports useful parallel work.

For parallel implementation, use persistent shared owners for bounded
independent lanes through `task` with the `worker` agent and `effort: max`.
Each brief states the checkout and dirty baseline, exact ownership, interfaces
and dependencies, applicable instructions, current evidence, and independent
acceptance checks.
...
Use `hub` to inspect, wait for, mail, cancel, or revive persistent shared
workers, retaining their stable agent identity across parent turns. Use an
isolated `task` only when a retained patch and parent non-mutation are required.
With `task.isolation.apply: false`, a completed isolated worker is not revivable;
route later corrections to a persistent shared owner or start a new explicitly
bounded isolated task.
...
handle, and collect its exit status and output before dependent work. When a wrapper controls the outer wait, keep that wait long enough to collect the
command while keeping each blocking interval within 60 seconds. Configure higher overall runtime limits
...
When a task warrants substantial planning in an idle Herdr-owned interactive
OMP pane, call `plan_enter` as the only tool in the response, then emit only the
returned marker. The detached helper enters OMP's native Plan and submits the
private continuation once. Outside Herdr, ask the user to type `/plan`; do not
emulate Plan or force every session to start there. Keep routine work in Build.

Native Plan proposals are written to `local://<slug>-plan.md` and submitted by
writing the slug to `xd://propose`. The profile blocks that root proposal while
RevDiff reviews the exact pinned artifact. Address every returned annotation in
the same root Plan session and resubmit; OMP's native approval overlay is the
only Plan exit surface. Never request or answer approval in prose.
