...
Reviewer agent definitions deny all tools and allow only bounded repository
read, glob, and search operations. Keep the behavioral prohibition on edits in
every brief as well, and verify from the reviewer transcript that it performed
...
Spawn configured reviewers through `task` with their exact agent names and keep
their returned `agent://` identities. Launch all selected reviewers before
waiting through `hub`, and collect a fresh completion from every selected
reviewer before validating findings or making a correction. Repeated old
completed status is stale lifecycle state, not a fresh result.
