...
OMP reports the absolute base directory when this skill is loaded.
...
4. OMP reads annotations and addresses each one
...
When OMP runs inside an agterm session, the launcher opens revdiff in agterm's native full-pane overlay — its first-choice backend, checked ahead of tmux. That branch needs `AGTERM_SESSION_ID` and `AGTERM_SOCKET` in the launcher's environment.
