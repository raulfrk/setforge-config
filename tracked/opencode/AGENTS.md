...
When a task warrants substantial planning, call `setforge_plan_enter` without asking the
user to switch manually. Make `setforge_plan_enter` the only tool call in that response,
then continue the same request under the Plan agent. Keep routine work in Build.

A completed Plan response must contain exactly one
`<proposed_plan>...</proposed_plan>` block. The profile reviews that block in
RevDiff before it is accepted. Address every returned annotation in a complete
replacement and keep the review loop active until the user quits cleanly. After
the reviewed plan is approved, call `setforge_plan_exit` as the only tool in that
response; never bypass its permission prompt.
