...
def create_baseline_repo(
    ...
    repo.mkdir()
    review_file = repo / "opencode-plan.md"
    render_single(source, review_file, mapping, width, padding)
    subprocess.run(["git", "init", "-q"], cwd=repo, check=True)
    subprocess.run(["git", "add", "opencode-plan.md"], cwd=repo, check=True)
    subprocess.run(
        [
            "git",
            "-c",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="plan-review-view-") as temp_value:
        ...
        new_view = temp / "opencode-plan.md"
        new_map = temp / "opencode-plan.map.json"
