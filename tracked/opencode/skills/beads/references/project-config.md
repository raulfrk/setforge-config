...
| --- | --- | --- | --- | --- | --- |
| `epic.mode=milestone` | Workflow convention | Epics group reviewable outcomes; leaf Beads own implementation. | profile Beads workflow | `database` | `bd config show` |
| `commit.mode=per-bead` | Workflow convention | Each leaf Bead produces one coherent commit. | profile Beads workflow | `database` | `bd config show` |
| `commit.format=type-prefix` | Workflow convention | Commit subjects use established `feat:`, `fix:`, `docs:`-style prefixes and omit private Bead IDs. | profile Beads workflow | `database` | `bd config show` |
| `custom.workflow.decomposition=reviewer-friendly` | Workflow convention | Split work only when independently reviewable pieces improve review. | profile Beads workflow | `database` | `bd config show` |
