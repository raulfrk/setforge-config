...
If these values are absent, detached caller identity is unavailable. Continue
only when the user supplied an explicit Herdr session and pane ID that resolves
to one live OpenCode pane. Otherwise report that the caller cannot be resolved
safely and stop. Never match by terminal title, display name, pane order, or the
UI-focused pane.

Recheck the exact immutable pane immediately before a mutating command because
pane IDs may change after moves. Prefix detached commands with
`herdr --session <resolved-session>` and target the explicit pane ID instead of
using `--current`.
